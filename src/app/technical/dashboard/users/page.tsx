...
import { deleteUsers, fetchUsers } from "@/store/technical/users/user-slice"
import { IUserFinalData } from "@/store/technical/users/users-types"
import UserModal from "@/lib/components/modal/UserModal"
...
function users() {
    ...
    const { users } = appSelector((store) => store.users)
    ...
    let filteredData = users.filter((user) => user.userName.includes(searchText) || user.role.includes(searchText))
            ...
            <div className=" overflow-x-auto">
                <div className="min-w-full inline-block align-middle">
                    <div className="relative  text-gray-500 focus-within:text-gray-900 mb-4 flex justify-between">
                        ...
                        <input onChange={(e) => setSearchText(e.target.value)} type="text" id="default-search" className="block w-80 h-11 pr-5 pl-12 py-2.5 text-base font-normal shadow-xs text-gray-900 bg-transparent border border-gray-300 rounded-full placeholder-gray-400 focus:outline-none" placeholder="Search Category" />

                        <button onClick={openModal} className="bg-blue-500 text-white rounded-2xl p-1">+Add User</button>
                    </div>
                    <div className="overflow-hidden ">
                        ...
                        <table className=" min-w-full rounded-xl">
                            ...
                            <tbody className="divide-y divide-gray-300 ">
                                {filteredData.length > 0 && filteredData.map((users: IUserFinalData) => {
                                    return (


                                        <tr key={users?.id} className="bg-white transition-all duration-500 hover:bg-gray-50">
                                            <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900 "> {users?.id}</td>
                                            <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {users?.userName} </td>
                                            <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {users?.email}</td>
                                            <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {users?.role}</td>
                                            <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900"> {users?.createdAt ? new Date(users.createdAt.toString()).toLocaleDateString()
                                                : "Invalid Date"}</td>
                                            <td className=" p-5 ">
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    <button onClick={() => handleUserDelete(users?.id)} className="p-2 rounded-full  group transition-all duration-500  flex item-center">
