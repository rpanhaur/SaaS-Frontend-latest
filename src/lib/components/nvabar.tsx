      ...
      {isOpen && (
        <div className="md:hidden bg-white border-t shadow-sm">
          <div className="px-4 py-3 space-y-2">
            ...
            <MobileLink href="/signup" text="Sign Up" />
            <MobileLink href="/signin" text="Sign In" />
            <MobileLink href="/create-institute" text="Create Institute" highlight />
          </div>
        </div>
      )}
    </nav>
...
const NavLink = ({ href, text, highlight = false }: { href: string; text: string; highlight?: boolean }) => (
  <Link
    ...
    className={`text-gray-700 hover:text-blue-600 font-medium transition duration-200 ${
      highlight ? 'bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600' : ''
    }`}
  >
    {text}
  </Link>
...
const MobileLink = ({ href, text, highlight = false }: { href: string; text: string; highlight?: boolean }) => (
  <Link
    href={href}
    className={`block px-3 py-2 rounded text-gray-800 hover:bg-blue-100 transition font-medium ${
      highlight ? 'bg-blue-500 text-white hover:bg-blue-600' : ''
    }`}
  >
    {text}
  </Link>
